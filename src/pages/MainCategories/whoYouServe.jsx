...
import {
  ACTION_addServing,
  ACTION_getServing,
  ACTION_updateServing,
  ACTION_deleteServingSetup,
  ACTION_getServingSetup,
  ACTION_updateServingSetup,
...
const whoYouServe = () => {
  ...
  const { whoYouServe,whoYouServeItems } = useSelector((state) => state.mainCategory);
  // begin item states
  ...
  // Starting whoYouServe Service states
  ...
  const addNewItemHandler = () => {
    ...
    dispatch(ACTION_addServing(reqPacket));
    resetItem();
  };
  const updateItemHandler = (e) => {
    ...
    dispatch(ACTION_updateServing(reqPacket));
    resetEditItem();
  };
  ...
  const deleteItemHandler = () => {
    ...
    dispatch(ACTION_deleteServingSetup(reqPacket));
    setShow(false);
    setDeleteItem(null);
  };
  ...
  const updateWhoYouServeHandler = () => {
    if (!title) return;

    const dtoServeringOptions = [];
    whoYouServeItems.forEach((_item) => {
      dtoServeringOptions.push({
        id: _item.id,
      });
    });
    ...
    dispatch(ACTION_updateServingSetup(reqPacket));
  };

  useEffect(() => {
    dispatch(ACTION_getServing());
    dispatch(ACTION_getServingSetup());
  ...
  useEffect(() => {
    if( !_.isEmpty(whoYouServe) ) {
      setTitle(whoYouServe.title);
      setDescription(whoYouServe.description);
      setIsDisplayTitle(whoYouServe.isDisplayTitle);
      setIsDisplayDescription(whoYouServe.isDisplayDescription);
      setIsDropdown(whoYouServe.isDropdown);
      setIsCheckBox(whoYouServe.isCheckBox);
      setIsMultipleSelection(whoYouServe.isMultipleSelection);
      setIsRequired(whoYouServe.isRequired);
      setStatus(1);
              ...
              <div className="col-md-5">
                ...
                <div className="creat-main-category-submit  mt-5">
                  <button
                    className="btn"
                    type="button"
                    onClick={updateWhoYouServeHandler}
                  >
                    UPDATE
                  </button>
                </div>
              ...
              <div className="col-md-7">
                <div className="table-container">
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {whoYouServeItems && whoYouServeItems.length
                        ? whoYouServeItems.map((item,index) =>
