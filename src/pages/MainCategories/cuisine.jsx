...
import {
  ACTION_addCuisine,
  ACTION_getCuisine,
  ACTION_updateCuisine,
  ACTION_deleteCuisine,
  ACTION_updateCuisineSetup,
  ACTION_getCuisineSetup,
...
const cuisine = () => {
  ...
  const { cuisine,cuisineItems } = useSelector((state) => state.mainCategory);
  // begin item states
  ...
  // Starting cuisine Service states
  ...
  const addNewItemHandler = () => {
    ...
    dispatch(ACTION_addCuisine(reqPacket));
    resetItem();
  };
  const updateItemHandler = (e) => {
    ...
    dispatch(ACTION_updateCuisine(reqPacket));
    resetEditItem();
  };
  ...
  const deleteItemHandler = () => {
    ...
    dispatch(ACTION_deleteCuisine(reqPacket));
    setShow(false);
    setDeleteItem(null);
  };
  ...
  const updateCuisineHandler = () => {
    if (!title) return;

    const cuisinesIds = [];
    cuisineItems.forEach((_item) => {
      cuisinesIds.push({
        id: _item.id,
      });
    });
    ...
    dispatch(ACTION_updateCuisineSetup(reqPacket));
  };

  useEffect(() => {
    dispatch(ACTION_getCuisine());
    dispatch(ACTION_getCuisineSetup());
  ...
  useEffect(() => {
    if( !_.isEmpty(cuisine) ) {
      ...
      setIsDisplayTitle(cuisine.isDisplayTitle);
      setIsDisplayDescription(cuisine.isDisplayDescription);
      setIsDropdown(cuisine.isDropdown);
      setIsCheckBox(cuisine.isCheckBox);
      setIsMultipleSelection(cuisine.isMultipleSelection);
      setIsRequired(cuisine.isRequired);
      setStatus(1);
              ...
              <div className="col-md-7">
                <div className="table-container">
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {cuisineItems && cuisineItems.length
