...
const menuService = () => {
  ...
  const { menuService,menuServiceItems } = useSelector((state) => state.mainCategory);
  // begin item states
  ...
  // Starting menuService Service states
  ...
  const addNewItemHandler = () => {
    ...
    dispatch(ACTION_addMenu(reqPacket));
    resetItem();
  };
  const updateItemHandler = (e) => {
    ...
    dispatch(ACTION_updateMenu(reqPacket));
    resetEditItem();
  };
  ...
  const deleteItemHandler = () => {
    ...
    dispatch(ACTION_deleteMenu(reqPacket));
    setShow(false);
    setDeleteItem(null);
  };
  ...
  const updateMenuServiceHandler = () => {
    if (!title) return;

    const dtoMenuOptions = [];
    menuServiceItems.forEach((_item) => {
      dtoMenuOptions.push({
        id: _item.id,
      });
    });
    ...
    dispatch(ACTION_updateMenuSetup(reqPacket));
  };

  useEffect(() => {
    dispatch(ACTION_getMenu());
    dispatch(ACTION_getMenuSetup());
  ...
  useEffect(() => {
    if( !_.isEmpty(menuService) ) {
      setTitle(menuService.title);
      setDescription(menuService.description);
      setIsDisplayTitle(menuService.isDisplayTitle);
      setIsDisplayDescription(menuService.isDisplayDescription);
      setIsDropdown(menuService.isDropdown);
      setIsCheckBox(menuService.isCheckBox);
      setIsMultipleSelection(menuService.isMultipleSelection);
      setIsRequired(menuService.isRequired);
      setStatus(1);
              ...
              <div className="col-md-5">
                ...
                <div className="creat-main-category-submit  mt-5">
                  <button
                    className="btn"
                    type="button"
                    onClick={updateMenuServiceHandler}
                  >
                    UPDATE
                  </button>
                </div>
              ...
              <div className="col-md-7">
                <div className="table-container">
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {menuServiceItems && menuServiceItems.length
                        ? menuServiceItems.map((item,index) =>
